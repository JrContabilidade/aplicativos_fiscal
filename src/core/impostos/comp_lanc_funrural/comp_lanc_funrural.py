...
from .consulta_nfs_questor import NotaQuestor, consultar_nf_questor
    ...
    def processar(self):
        ...
        for nota_csv in notas_csv:
            if nota_csv.error:
                self._csv_result += (
                    nota_csv.linha
                    + f';colunas {nota_csv.error.columns} inválidas\n'
                )

            else:
                nota_encontrada = self._buscar_nota_questor(
                    notas_questor, nota_csv
                )
